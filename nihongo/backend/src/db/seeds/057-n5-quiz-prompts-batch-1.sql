...
--
-- Character-for-character the derivation from 055, run again.
--
-- It has to be a copy rather than a reference: a seed is recorded in
-- `seed_history` by filename and never runs a second time, so 055 cannot see
-- the 40 sentences that arrived after it. The derivation itself is written to
-- be re-runnable — it reads whatever authored sentences exist and every insert
-- is `ON CONFLICT DO NOTHING` — so running it again picks up only what is new
-- and leaves the existing questions untouched.
--
-- Every later batch of sentences needs its own copy of this file for the same
-- reason. If that becomes tiresome, the derivation is the thing to move into a
-- code seed that can be called; it is not worth doing for the second batch.
--
-- Requires the batch 1 tokens, which 056 inserts — word-order chips and cloze
-- spans are cut from `sentence_tokens`, not from the sentence text. 056 sorts
