...
--
-- The same derivation as 055 and 057, run a third time. See 057's header for
-- why each batch needs its own copy: a seed is recorded by filename and never
-- runs twice, so an earlier one cannot see sentences that arrive after it.
--
-- Three copies is the point at which this stops being tolerable. The next batch
-- should move the derivation into a code seed that takes the batch as an
-- argument, rather than a fourth verbatim copy of 120 lines of SQL.
--
-- Requires the batch 2 tokens from 058, which sorts first.
